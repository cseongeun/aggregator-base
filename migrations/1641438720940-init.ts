...
export class init1641438720940 implements MigrationInterface {
    name = 'init1641438720940'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`nf_token\` (\`status\` tinyint NOT NULL DEFAULT 1, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp NULL, \`id\` bigint NOT NULL AUTO_INCREMENT, \`address\` varchar(255) NOT NULL, \`name\` varchar(255) NOT NULL, \`uri_type\` enum ('HTTP', 'IPFS') NOT NULL, \`token_id\` int NOT NULL, \`token_uri\` varchar(255) NULL, \`token_uri_data\` varchar(255) NULL, \`image_or_animation_uri\` varchar(255) NULL, \`protocol_id\` bigint NOT NULL, INDEX \`idx_nfToken_2\` (\`address\`), INDEX \`idx_nfToken_1\` (\`protocol_id\`, \`address\`, \`token_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
